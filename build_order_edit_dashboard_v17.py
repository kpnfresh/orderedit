...
# so the script can live anywhere but always writes here. Both files MUST
# land in this same folder together: index.html does a relative fetch
# ('./order_edit_dashboard_data.json'), so if the two ever end up in
# different folders (or only one gets committed/pushed) the site breaks.
...
def main():
    ...
    OUTPUT_DATA_JSON = os.path.join(OUTPUT_DIR, "order_edit_dashboard_data.json")
    try:
        with open(OUTPUT_DATA_JSON, "w", encoding="utf-8") as f:
            json.dump(data_blob, f, separators=(",", ":"), allow_nan=False)
    except ValueError as e:
        os.remove(OUTPUT_DATA_JSON)  # don't leave a partially-written/broken file behind
        bad_paths = _find_non_finite(data_blob)
        raise SystemExit(
            "ERROR: refusing to write order_edit_dashboard_data.json -- it contains "
    ...
    print(f"Dashboard shell written to {OUTPUT_HTML}")
    print(f"Data file written to {OUTPUT_DATA_JSON}  (upload BOTH files, same folder, to GitHub)")
...
// ---------- bootstrap: fetch the data file, then start the dashboard ----------
fetch('./order_edit_dashboard_data.json')
  .then(r=>{
  ...
  .catch(err=>{
    ...
    if(ov){
      ov.innerHTML = '<div style="max-width:360px;text-align:center;font-family:var(--f)">'
        ...
        + '<div style="font-size:12.5px;color:#7b8499;line-height:1.5">Make sure order_edit_dashboard_data.json is in the same folder as this file, and that this page is being opened over http(s) (e.g. GitHub Pages) rather than double-clicked as a local file.</div>'
        + '</div>';
    }
  });
</script>
